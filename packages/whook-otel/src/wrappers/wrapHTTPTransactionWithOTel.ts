...
export interface WhookOTelHTTPTransactionDependencies {
  oTelTracer: WhookOTelTracerService;
  log?: LogService;
}

interface SpanContextPayload {
  ...
  return wrapInitializer(
    async (
      ...
      return async (
        ...
        const transactionContext = trace.setSpan(
          parentContext,
          transactionSpan,
        );
...
async function startTransaction(
  ...
  try {
    ...
    const parsingDurationMs = Math.max(
      0,
      routingDurationMs - executionDurationMs,
    );
    ...
    transactionSpan.setAttribute(
      'whook.router.routing.duration_ms',
      routingDurationMs,
    );

    return response;
  } catch (err) {
    transactionSpan.recordException(err as Error);
