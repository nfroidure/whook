...
describe('initBaseURL', () => {
  ...
  test('should work with a base URL in env', async () => {
    const BASE_URL = await initBaseURL({
      ENV: {
        PUBLIC_URL: 'https://example-env.com',
      },
      CONFIG: {
        name: 'project',
        baseURL: 'https://example.com',
    ...
    expect({
     ...
     {
       "BASE_URL": "https://example-env.com",
       "logCalls": [
         [
           "debug",
           "🈁 - Generated the BASE_URL constant "https://example-env.com".",
         ],
       ],
     }
    `);
