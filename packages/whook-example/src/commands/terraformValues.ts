...
import { OpenAPI, OpenAPIMethod, OpenAPIPath } from 'ya-open-api-types';
  ...
  return async (args): Promise<void> => {
    ...
    for (const handlerName of Object.keys(DEFINITIONS.configs)) {
      ...
      definitions.push({
        ...
        method:
          baseDefinition.type === 'route'
            ? baseDefinition.method.toLowerCase()
            : '',
        path:
          baseDefinition.type === 'route'
    ...
    if (type === 'envvars') {
      const env = (PROXIED_ENV_VARS as string[]).reduce(
        (finalEnv, key: string) => ({ ...finalEnv, [key]: ENV[key] }),
        {},
      );
