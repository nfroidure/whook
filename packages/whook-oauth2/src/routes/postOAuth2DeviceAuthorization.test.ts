...
import { type WhookOAuth2DeviceCodeService } from '../services/oAuth2DeviceCodeGranter.js';
...
describe('postOAuth2DeviceAuthorization', () => {
  ...
  beforeEach(() => {
    ...
    oAuth2DeviceCode.create.mockReset();
    time.mockReset();
    log.mockReset();
  ...
  test('should create a device authorization challenge', async () => {
    ...
    oAuth2DeviceCode.create.mockResolvedValueOnce({
    ...
    expect({
      ...
      oAuth2DeviceCodeCreateCalls: oAuth2DeviceCode.create.mock.calls,
      logCalls: log.mock.calls.filter(([type]) => !type.endsWith('stack')),
    }).toMatchInlineSnapshot(`
     {
       "logCalls": [],
       "oAuth2DeviceCodeCreateCalls": [
         [
           {
             "clientId": "root_app_id",
             "scopes": [
