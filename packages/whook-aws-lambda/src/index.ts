...
export async function runBuild(
  ...
  try {
    ...
    await processOperations(
      { NODE_ENV, BUILD_PARALLELISM, log, $autoload, buildInitializer },
      operations,
    );
    await $.destroy();
    process.exit();
...
async function processOperations(
  ...
  await Promise.all(
    operations
      ...
      .map(operation =>
        buildAnyLambda(
          { NODE_ENV, log, $autoload, buildInitializer },
          operation,
        ),
      ),
  );
...
async function buildAnyLambda(
  ...
  try {
    ...
    const lambdaPath = path.join('builds', NODE_ENV, finalEntryPoint);
    ...
    await Promise.all([
      copyStaticFiles({ log }, lambdaPath, whookConfig.staticFiles || []),
      ensureFileAsync(
        { log },
...
async function buildFinalLambda({ NODE_ENV, log }, lambdaPath, whookConfig) {
  const memoryFS = createFsFromVolume(new Volume());
  const compiler = webpack({
    entry: `${lambdaPath}/main.js`,
    target: 'node',
    mode: NODE_ENV.endsWith('production')
      ? // Sadly setting to 'production' here significantly slows
        // down the build. It may be due to UglifyJS which is clearly
        // not useful for this build, see https://webpack.js.org/concepts/mode/
        'production'
      : 'development',
    node: {
      __dirname: true,
    ...
    externals: whookConfig.externalModules,
    // devtool: 'source-map',
    output: {
    ...
    module: {
      rules: [
        ...
        {
          test: /\.(js|ts)$/,
          exclude: /node_modules(?!\/sencrop-mails)/,
          use: {
            loader: 'babel-loader',
            options: {
              presets: [
  ...
  const data = await new Promise((resolve, reject) => {
    compiler.run((err, stats) => {
      ...
      resolve(memoryFS.readFileSync(`${lambdaPath}/index.js`, 'utf-8'));
    });
  });
  await ensureFileAsync({ log }, `${lambdaPath}/index.js`, data, 'utf-8');
}

async function copyStaticFiles({ log }, lambdaPath, staticFiles = []) {
  await Promise.all(
    staticFiles.map(
      async staticFile =>
        await copyFiles(
          { log },
          path.join(__dirname, '..', 'node_modules', staticFile),
...
async function mkdirpAsync(path) {
